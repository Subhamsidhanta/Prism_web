...
frontend_origin = os.getenv("FRONTEND_ORIGIN", "https://prism-web-three.vercel.app")  # Vite default dev port
...
if not allow_origins:
    allow_origins = [
        "https://prism-web-three.vercel.app",
...
async def root():
    ...
    return {
        ...
        "model_name": getattr(mistral_llm, "model_name", "unknown")
    }
...
async def chat(request: ChatRequest):
    ...
    try:
        ...
        if not mistral_llm.is_ready():
            ...
            detail = (
                f"LLM not ready (mode={mode}, provider={provider}). "
                + ("For API mode ensure LLM_API_KEY is set and model/provider are valid." if mode == "api" else "Place a .gguf model in models/llm/ or switch to API mode.")
            )
...
async def model_status():
    ...
    return {
        ...
        "model_path": getattr(mistral_llm, "model_path", None),
        "context_size": getattr(mistral_llm, "n_ctx", None),
        "model_name": getattr(mistral_llm, "model_name", "unknown"),
        "mode": os.getenv("LLM_MODE", "local"),
        "provider": os.getenv("LLM_PROVIDER", "local"),
        "last_error": getattr(mistral_llm, "last_error", None),
    }
